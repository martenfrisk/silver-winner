<script lang="ts">
	// The app-graded word review: the original /practice session, unchanged.
	// It picks an exercise format per word from its SRS box and grades the
	// answer itself (see the ladder in $lib/practice-session).
	//
	// Lives in a component rather than the route because /practice now chooses
	// between this and SelfReviewSession on progress.selfReview.
	...
	/**
	 * Grades a recognition drill as a miss without a guess. A tapped option is
	 * indistinguishable from a lucky guess, so a learner who admits they don't
	 * know avoids inflating their box on a coin flip — see the header note in
	 * practice-session.ts on why the box drives exercise format.
	 */
	function idk() {
		if (!ex || !item || status !== 'answer') return;
		applyResult(false);
