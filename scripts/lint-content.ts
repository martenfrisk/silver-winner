...
// lessonOrder used to be derived inside course.ts, which meant importing it
// (progress.svelte.ts does, and the root layout imports that) pulled every
// exercise in the course onto every page. Emitting it here keeps course.ts
// the only file content authors touch, and lesson-order.test.ts fails the
// test gate if this file is stale.
{
	const lessons = course.flatMap((u) => u.lessons);
	const ids = lessons.map((l) => l.id);
	const optional = lessons.filter((l) => l.optional).map((l) => l.id);
	const path = join(import.meta.dirname, '../src/lib/data/lesson-order.ts');
	const list = (xs: string[]) => xs.map((id) => `\t${JSON.stringify(id)}`).join(',\n');
	const next =
	...
	if (!existsSync(path) || readFileSync(path, 'utf8') !== next) {
		...
		console.log(
			`↻ regenerated src/lib/data/lesson-order.ts (${ids.length} lessons, ${optional.length} optional)`
		);
	}
}
